...
from cerberus_proxy.guards.translator import _cached_translate
...
# CERBERUS_TRANSLATE_TIMEOUT for the module.


@pytest.fixture(autouse=True)
def _generous_translation_timeout(monkeypatch):
    monkeypatch.setenv("CERBERUS_TRANSLATE_TIMEOUT", "30")
...
async def test_translation_timeout_passes_through(monkeypatch):
    ...
    monkeypatch.setattr("cerberus_proxy.guards.input_guard.to_english", _raise_timeout)
    result = await InputGuard().scan("What is the annual leave policy?")
...
async def test_proxy_blocks_injection(session_factory, client, create_key):
    ...
    response = await client.post(
        "/v1/chat/completions",
        json=request_body,
        headers={"X-Cerberus-Key": api_key, "Authorization": "Bearer sk-customer-key"},
    )
