...
from cerberus_proxy.guards.base import ReasonCode, Severity
from cerberus_proxy.guards.output_guard import (
    OutputGuard,
    calculate_entropy,
    luhn_check,
)
...
async def test_proxy_redacts_credit_card_in_response(
    ...
    monkeypatch.setenv("CERBERUS_OUTPUT_GUARD_ACTION", "redact")
    ...
    response = await client.post(
        ...
        headers={"X-Cerberus-Key": api_key, "Authorization": "Bearer sk-customer"},
    )
    ...
    assert "CREDIT_CARD" in response.headers["X-Cerberus-Redacted"]
...
async def test_proxy_redacts_api_key_in_response(
    ...
    assert "OPENAI_API_KEY" in response.headers["X-Cerberus-Redacted"]
...
async def test_proxy_blocks_in_block_mode(
    ...
    monkeypatch.setenv("CERBERUS_OUTPUT_GUARD_ACTION", "block")
...
async def test_proxy_log_only_passes_through(
    ...
    monkeypatch.setenv("CERBERUS_OUTPUT_GUARD_ACTION", "log_only")
    ...
    assert "X-Cerberus-Redacted" not in response.headers
